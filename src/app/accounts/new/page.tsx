        ...
        <div className="mt-6 grid gap-3">
          {items.map((it) => (
            <Link
              key={it.href}
              href={it.href}
              className="group flex items-center gap-4 rounded-md border border-[var(--c-border)] bg-[var(--c-surface)] px-5 py-4 shadow-sm hover:bg-[var(--c-surface-soft)]"
            >
              <span className="flex h-10 w-10 items-center justify-center rounded-full bg-[var(--c-accent)]/10 text-base font-semibold text-[var(--c-accent)]">
                {it.icon}
              </span>
              <div className="flex-1">
                <div className="text-base font-semibold">{it.title}</div>
                <div className="mt-1 text-sm text-[var(--c-muted)]">{it.desc}</div>
              </div>
              <span className="text-[var(--c-faint)] group-hover:text-[var(--c-muted)]">→</span>
            </Link>
          ))}
        </div>
      </main>
