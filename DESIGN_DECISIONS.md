...
# v1.2 — 首頁總覽視覺層級與互動（2026-08-18）

> 上面 D1–D14 那輪（design-pass-2）已結案，內容保留不動。這一輪是獨立的第二次診斷，
> 編號 V1–V14 與 C1–C5，與 D 系列無對應關係。
> 方向：延續《帳本》，局部重做字體系統；範圍限首頁總覽。分支 `feat/v1.2-block1-hierarchy-and-chart`。
>
> 路徑：Mac 為 `~/ClaudeCode/projects/portfolio-tracker`（上方 Windows 路徑註記已不適用）。

## 這一輪的方法

診斷一律以「量到的數字」為準，不以「看起來如何」為準。過程中有三個我自己提出的
診斷被後續測量推翻，都已撤回並改變了做法：

- **V2「深色模式卡片邊界幾乎消失、淺色清楚」** — 實測深色 1.246:1、淺色 1.244:1，
  兩者相同。撤回。真正的問題不是邊界太弱，而是五個區塊戴同一頂帽子，因此 Block 1
  改成「減少卡片數量」而不是「加強邊框」。
- **V7「襯線只服務 hero 大數字」** — grep 出全站約 50 處。這推翻了我先前給使用者的
  成本估計（原稱移除襯線是小工程）。
- **「面積圖底部與資料無關」** — 誤判，`H - padB` 就是繪圖區底部＝`lo`。撤回。

## Block 1 — 三級層級 + 折線圖可讀性（V1, V2✗, V6 / C1–C5）
- 首頁改三級：一級摘要無容器、二級核心資料保留 surface 容器、三級靠 hairline。
- 四格關鍵指標脫離卡片殼，用負邊距抵銷內距讓左緣與 Hero 對齊（實測 88px → 68px）。
- 折線圖：`chart-scale.ts`（nice number 刻度、標籤密度依寬度、軸專用格式），28 條測試。
- 新增 `--c-grid`（原本用 `--c-border`，對卡片底 1.25:1 等同看不見）。
- 取捨：大盤對照圖的資料佔繪圖高度由 83% 降到約 70%，換整數刻度與可讀的 100 基準線。

## Block 2 — 字體與字級（V7）
- Space Grotesk → IBM Plex Sans；數字加 IBM Plex Mono；Newsreader 保留。
- 字級收成 8 級 token。**不用 Tailwind `@theme`**：會改動全站 316 處 utility。
- 踩到的坑（都已修）：`text-[var(--fs-*)]` 被 Tailwind 判成顏色（型別不明時它猜 color），
  必須寫 `text-[length:var(...)]`；`--font-mono` 與 Tailwind utility 撞名，改名
  `--font-figures`；手機「投入成本」因等寬變寬被切字（163px 可用 / 164px 需要），
  改成響應式字級。

## Block 3 — 數字與圖表基本功（V3, V4, V5）
- 幣別規則：同單位群組的最上層宣告一次。四格去掉 NT$（量過：主格補上後八位數要 198px，
  可用只有 163px），表格改在表頭標。
- 持倉表格 colgroup 明寫欄寬；配置長條改撐滿欄寬，刻度維持 0–100% 絕對值
  （滿格＝單一帳戶吃掉整個組合，那段空白本身是集中度資訊）。
- 圓環中心字級降一級：孔徑 96px，26px 下最寬字串 104px，本來就壓在圓環上。

## Block 4 — 資料互動（V9, V10, V11）
- 選中語彙收斂成兩套（互斥選擇 / 獨立開關），詳見 `shared.tsx` 的 `PICK_ON`。
- 排序標籤桌機手機統一；藥丸列補「排序」標籤與 `role="group"`。
- 修掉配置類別列 hover/click 共用 state 的 bug（滑鼠永遠釘不住）。
- **V9 原案被測量否決再重做**：原本只想把選取狀態提上去做連動，但實測 1280×800 下
  持倉表格 y 554–951、資產配置 y 1466–1912，相距 515px，效果會落在畫面外。
  改為先搬版面（配置移到持倉旁邊，斷點 1180px 由 760+344 的寬度預算決定）再接連動。

## Block 5 — 動效節奏 / 密度 / 長頁導引（V8, V12, V13, V14）
- 五區塊依序進場；`main:has(.ledger)` 關掉外層那層整頁淡入。
- 手機持倉卡 133 → 106px；新增 `--c-row-hover`（1.073 → 1.245:1）。
- 手機捲過 Hero 後浮出精簡摘要（桌機不做）。

## 已知未驗證 / 未解決

- **StickySummary 的捲動觸發未實測**。自動化瀏覽器讓 `document.hidden` 恆為 true，
  未渲染的文件會跳過 update the rendering，IntersectionObserver 回呼因此不送達。
  已驗到顯示時的位置、尺寸、內容、層級與無水平溢出，切換本身待真實瀏覽器確認。
- **收尾檢查時整個 `/demo` 頁面子樹在自動化瀏覽器裡不會 hydrate**（574 元素只有 31 個
  掛得上 React）。已用線上 v1.1.0 站對照確認同樣症狀，屬環境問題不是本輪迴歸；
  但也因此，排序、切換器等互動在收尾階段無法重測。
- **手機上配置↔持倉的連動可見範圍仍有限**：堆疊後配置緊接持倉，但一次仍只看得到
  最後一兩張持倉卡。要真的解決得改成「點類別→篩選持倉」。
- **demo 資料沒有封存帳戶**，持倉「配置」欄 `share == null → —` 那條分支未被實際渲染。
- 既有問題，本輪未處理：`/login` 輸入框 14px（iOS 會自動縮放，需 ≥16px）；
  CSP nonce 造成的 hydration mismatch 每頁都報（v1.0 CSP 那輪留下）。
